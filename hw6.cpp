// This program reads and processes the input student data file, and then 
// writes the process results to an output student report file
// Everything is built to the spec 
...
int main() {
	...
	string input_file_name = "ass6data.txt";
	string output_file_name = "student_results.txt";
	string file_string = "";
	int stud_num = 1, 
		 file_val = 0, 
		 assign_sum = 0, 
	...
	{
		letter_grade_mod = '+';
		//outputFile << letter_grade_mod << endl;
	}
	...
	cout << "Data and Results compiled into output file: " << output_file_name
		  << endl;
	inputFile.close();
